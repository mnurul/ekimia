...
class Pesanan extends CI_Controller
{
	...
	function konfirmasi_pembayaran($no_pesanan)
	{
		...
		$plg_id = $this->session->userdata('plg_id');
		$cekNo = $this->Model_app->edit_data('pesanan', array('no_pesanan' => $no_pesanan))->row();

		// var_dump($cekNo->status_pesanan);
		if ($cekNo->status_pesanan != 0) {
			$data['konfirmasiProduk'] = $this->input->post('konfirmasiProduk');
			// $data['konfirmasiProduk']=null;
			if ($data['konfirmasiProduk'] = "Produk Sudah Sampai") {
				$data['konfirmasiProduk'] = 6;
			}
		}

		// var_dump($data['konfirmasiProduk'], $cekNo->no_pesanan);
		if ($no_pesanan == $cekNo->no_pesanan && $cekNo->status_pesanan == 3) {
			$this->db->set('status_pesanan', 6, FALSE);
		...
		$query = $this->db->get_where('pesanan', array('no_pesanan' => $no_pesanan));
