...
class Testimoni extends CI_Controller
{
	...
	function index()
	{
		...
		$data['data_comment'] = $this->Model_app->get_comment()->result();
		// var_dump($data['data_comment']);
		// die();
	...
	function comment()
	{
		$data['idtesti'] = $this->input->post('idtesti');
		$data['plg_id'] = $this->session->userdata('plg_id');
		$data['email'] = $this->session->userdata('plg_email');
		$data['isi_comment'] = $this->input->post('isi_testi');
		$data['status_comment'] = 1;
		$data['tgl_comment'] = date('Y-m-d H:i:s');
		// var_dump($data['idtesti']);
		// die();

		$this->Model_app->insert_data('comment', $data);
