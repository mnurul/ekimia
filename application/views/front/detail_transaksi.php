					...
					<div class="sidebar-item product-info" style="width: 50%;">
						...
						<div class="information-layout">
							...
							<form enctype="multipart/form-data" method="post" action="<?= base_url('pesanan/konfirmasi-pembayaran/') . $data_transaksi->no_pesanan ?>">
								...
								<input type="text" name="jumlah_bayar" value="<?php if ($data_pembayaran)  echo $data_pembayaran->bayar_nominal;
																				else echo $data_transaksi->grandtotal;  ?>">
								<?php if ($data_transaksi->status_pesanan == 3) : ?>
									<label>Konfirmasi Produk</label>
									<input type="text" name="konfirmasiProduk" value="Produk Sudah Sampai">
								<?php endif ?>
								<!-- /INFORMATION LAYOUT ITEM -->
								<!-- <?php if ($data_transaksi->status_pesanan == 3) : ?>
									<form method="post" action="<?= base_url('pesanan/konfirmasiProduk/') . $data_transaksi->no_pesanan ?>">
										<label>Konfirmasi Produk</label>
										<input type="text" name="konfirmasiProduk" value="Produk Sudah Sampai">
										<button class="button primary" type="submit" onclick="myFunction()" style="width: 200px">Konfirmasi3</button> &nbsp;&nbsp;
									</form>
								<?php endif ?> -->
								...
								<div style="display: flex">
									<?php if ($data_transaksi->status_pesanan == 3) { ?>
										<button class="button primary" type="submit" onclick="myFunction()" style="width: 200px">Konfirmasi</button> &nbsp;&nbsp;
									<?php } elseif ($data_transaksi->status_pesanan == 0 || $data_transaksi->status_pesanan == 1) { ?>
...
<script>
	function myFunction() {
		confirm("Bener Produk Kamu Sudah Sampai?");
	}
</script>